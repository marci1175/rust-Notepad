...
impl eframe::App for TemplateApp {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        let can_autosave = self.autosave;
        if can_autosave && self.autosave_place.is_file() /*.is_file() is there to check if its not empty */ {
            let autosave_path = self.autosave_place.clone();
            let text_editor_contains = self.label.clone();
            std::thread::spawn(move || loop {
                ...
                let mut file = OpenOptions::new()
                        ...
                        match write!(file ,"{}", text_editor_contains){
                            Ok(_) => {},
                            Err(e) => {
            ...
            egui::menu::bar(ui, |ui| {
                ...
                if ui.button("Save As").clicked() {
                    ...
                    if let Some(file_path) = files {
                        self.autosave_place = file_path.clone();
                        let mut file = OpenOptions::new()
                            .create(true)
                            .write(true)
                            .open(file_path)
